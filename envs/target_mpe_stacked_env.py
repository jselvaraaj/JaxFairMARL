...
class StackedTargetMPEEnvironment(TargetMPEEnvironment):
    ...
    def reset(
        self, *args, **kwargs
    ) -> tuple[MultiAgentObservation, MultiAgentGraph, MPEStateWithBuffer]:
        ...
        for agent in self.agent_labels:
            obs_buffer[agent] = jnp.zeros(
                (self.stack_size, self.observation_space_for_agent(agent).shape[-1])
            )
            ...
            non_equivariant_nodes_buffer[agent] = jnp.zeros(
                (self.stack_size,)
                + multi_agent_graph[agent].non_equivariant_nodes.shape
            )

            obs_buffer[agent] = stack_arrays(multi_agent_obs[agent], obs_buffer[agent])
            multi_agent_obs[agent] = obs_buffer[agent]

            equivariant_nodes_buffer[agent] = stack_arrays(
                multi_agent_graph[agent].equivariant_nodes,
                equivariant_nodes_buffer[agent],
            )
            _equivariant_nodes = equivariant_nodes_buffer[agent].swapaxes(0, 1)

            non_equivariant_nodes_buffer[agent] = stack_arrays(
                multi_agent_graph[agent].non_equivariant_nodes,
                non_equivariant_nodes_buffer[agent],
            )
            _non_equivariant_nodes = non_equivariant_nodes_buffer[agent].swapaxes(0, 1)

            multi_agent_graph[agent] = (
                multi_agent_graph[agent]
                ._replace(equivariant_nodes=_equivariant_nodes)
                ._replace(non_equivariant_nodes=_non_equivariant_nodes)
            )

        mpe_state = MPEStateWithBuffer(
            *mpe_state,
    ...
    def _step(
        ...
        multi_agent_obs, multi_agent_graph, mpe_state, reward, done, info = (
            super()._step(key, state_without_buffer, actions)
        )
